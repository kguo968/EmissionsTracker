...
getGHG <- function(x) {
  ...
  ghg_factor_order <-
    c("Total CO2",
      ...
      "Totals")
  total_ghg <-
    total_ghg %>% mutate(Gas = factor(Gas, levels = ghg_factor_order)) %>% arrange(Gas)
  total_ghg
}
...
cleanSheet <- function(x) {
  ...
  if (col_len == 11) {
    emissions$`Lowest level?`[is.na(emissions$`Lowest level?`)] <-
      "False"
    emissions[, -c(1, 2, 11)] <-
      lapply(emissions[,-c(1, 2, 11)], function(x)
        unlist(as.numeric(x)))
  } else {
    emissions[, -c(1, 2)] <-
      lapply(emissions[,-c(1, 2)], function(x)
        unlist(as.numeric(x)))
  }
  
  emissions[rowSums(is.na(emissions)) > 0, 4:9] <- 0
  emissions <-
    emissions[apply(emissions[,-c(1:2, 10, 11)], 1, function(x)
      ! all(x == 0)), ]
  emissions[, col_len + 1] <-
...
fixAgricultureEmissions <- function(emissions) {
  ...
  emissions$`Link to CRF import file` <-
    stri_replace_all_regex(emissions$`Link to CRF import file`,
                           "^3.1",
                           replacement = "3.A")
  
  # Name on tab change
  emissions$`Name on tab` <-
    stri_replace_all_regex(emissions$`Name on tab`,
                           "^\\[3\\.\\d",
                           replacement = "[3.A")
...
fixAgricultureTime <- function(time) {
  ...
  time$`Link to CRF import file` <-
    str_trim(time$`Link to CRF import file`)
  
  time$`Link to CRF import file` <-
    stri_replace_all_regex(time$`Link to CRF import file`,
                           "^3.1",
                           replacement = "3.A")
  ...
  time$`Name on tab` <- stri_replace_all_regex(
    time$`Name on tab`,
    "^\\[3\\.[A-B][\\.\\d]*",
