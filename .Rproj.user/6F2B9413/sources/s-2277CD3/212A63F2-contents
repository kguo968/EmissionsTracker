...
server <- function(input, output, session) {
  ...
  category_change <- reactive({
    ...
    if (level() <= 1) {
      ...
      if (!all(!duplicated(emission_sector$`Name on tab`)) |
        ...
        if (length(unique((substr(emission_sector$`Link to CRF import file`, 
                              1, 
                              stri_locate_first_fixed(emission_sector$`Link to CRF import file`, " ") -1)))) != 1) {
          emission_sector <-
            Sector_Frame$data[Sector_Frame$data[[10]] == isolate(level()) + 1 &
                                substr(Sector_Frame$data[[1]],
                                       1,
  ...
  time_sector_change <- reactive({
    if (level() == 1) {
      ...
      switch(Category_Selector$selected,
      ...
      # If multiple instances of same Name on tab, further filter by Code
      if (dim(time_sector)[1] > 1) {
        time_sector <- time_sector %>% 
          filter(substr(`Link to CRF import file`,
                        1,
                        stri_locate_first_fixed(time_sector$`Link to CRF import file`, " ") - 1) == check$value)
        
        # If data frame is empty, re-filter directly from time_series_clean
        if (dim(time_sector)[1] == 0) {
          time_sector <- time_series_clean %>% 
            filter(substr(`Link to CRF import file`,
                          1,
                          stri_locate_first_fixed(time_series_clean$`Link to CRF import file`, " ") - 1) == check$value)
        }
      }
    }
  ...
  output$DataFrame <- renderDT({
    ...
    # Comparison_Frame$Totals <- round(Comparison_Frame$Totals, 2)
    # browser()
